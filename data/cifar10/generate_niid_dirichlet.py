...
try:
    ...
    mlrawdata = fetch_openml(name, data_home='./data.pkl')
...
# mlrawdata.data = mlrawdata.data.astype(np.int)
...
for beta in betas:
    ...
    # mldata (batch_size; width)
    mldata = [np.moveaxis(np.reshape(oneclass, [-1,3, 32,32]),1,-1) for oneclass in mldata]

    ### show images
    # figure,axs = plt.subplots(ncols=10,nrows=10)
    # for i in range(10):
    #     for j in range(10):
    #         for
    #         axs[i][j].imshow(mldata[i][j])
    # plt.show()
