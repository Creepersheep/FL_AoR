...
axs[0].plot(VALID_RANGE, data[data_sl.index(dict(betaValues=1,tierTimes=20,methods='aog',utiers=0,model='cifar10'))],label='LESSON')
axs[0].plot(VALID_RANGE, data[data_sl.index(dict(betaValues=1,tierTimes=20,methods='aog',utiers=1,model='cifar10'))],label='FedCS')
axs[0].plot(VALID_RANGE, data[data_sl.index(dict(betaValues=1,methods='avg',model='cifar10'))],label='FedAvg') # beta=1
...
axs[1].plot(mnist_valid_range, data[data_sl.index(dict(betaValues=1,tierTimes=20,methods='aog',utiers=0,model='mnist'))][:mnist_range],label='LESSON')
axs[1].plot(mnist_valid_range, data[data_sl.index(dict(betaValues=1,tierTimes=20,methods='aog',utiers=1,model='mnist'))][:mnist_range],label='FedCS')
axs[1].plot(mnist_valid_range, data[data_sl.index(dict(betaValues=1,methods='avg',model='mnist'))][:mnist_range],label='FedAvg')
...
axs[3].plot(np.arange(mnist_range)*20*TEST_frequency, data[data_sl.index(dict(betaValues=1,tierTimes=20,methods='aog',utiers=0,model='mnist'))][:mnist_range],  label='LESSON')
...
for k,beta in enumerate(betaValues[0:2]):
    for j,deadline in enumerate(var.tierTimes):
        axs[k].plot(VALID_RANGE,
                    data[setting_list.index(dict(betaValues=beta,tierTimes=deadline,utiers=0,methods='aog',model='cifar10'))],
...
for k,beta in enumerate(betaValues[0:2]):
    for j,deadline in enumerate(var.tierTimes):
        ...
        axs[k].plot(np.arange(f7fit)*deadline*TEST_frequency,
                    data[setting_list.index(dict(betaValues=beta,tierTimes=deadline,utiers=0,methods='aog',model='cifar10'))][:f7fit],
                    label='\u03C4='+str(deadline)+'s')
        axs[k].grid()
        axs[k].set(xlabel='Time (s)\n(' + chr(97 + k) + ') \u03B2=' + str(beta), ylabel='Test accuracy')
axs[0].legend()
