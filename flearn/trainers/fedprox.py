    ...
    def train(self):
        ...
        for i in range(self.num_rounds):
            ...
            if i % self.eval_every == 0:
                ...
                # load model from latest for test
            ...
            active_clients = np.random.choice(selected_clients,
                                              round(self.clients_per_round * (1 - self.drop_percent)), replace=False)

            csolns = []  # buffer for receiving client solutions

            # inner_opt from pgd
            # latest_model: model parameters
            # client_model: flearn.models.synthetic.mclr.Model
            ...
            for idx, c in enumerate(selected_clients.tolist()):
                ...
                # soln contain model parameters and weight (number of samples)
                if c in active_clients:
                    soln, stats = c.solve_inner(num_epochs=self.num_epochs, batch_size=self.batch_size)
                else:
                    # soln, stats = c.solve_iters(num_iters=np.random.randint(low=1, high=total_iters),
                    # batch_size=self.batch_size)
                    soln, stats = c.solve_inner(num_epochs=np.random.randint(low=0.9, high=self.num_epochs),
                                                batch_size=self.batch_size)
