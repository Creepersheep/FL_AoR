...
# 3 models
# client_model: model for the global model for test, binding with optimizer
    # inner_opt: optimizer for client_model
# clients[client_model]: list of models for each client
...
class BaseFedarated(object):
    def __init__(self, params, learner, dataset):
        ...
        # client_model parameters to latest_model
    ...
    def test(self):
        ...
        # must assign the latest_model value to client_model to test data.pkl on clients
        # otherwise, data.pkl is tested on the last client's model (single client)
        # single client result still keeps increasing but also unstable
        self.client_model.set_params(self.latest_model)
