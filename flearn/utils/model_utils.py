...
def batch_data(data, batch_size):
    '''
    data.pkl is a dict := {'x': [numpy array], 'y': [numpy array]} (on one client)
    returns x, y, which are both numpy array of length: batch_size
    '''
    data_x = data['x']
    data_y = data['y']

    # randomly shuffle data.pkl
    np.random.seed(100)
    ...
    # this one is not sgd, but mini-batch
...
def epoch_batch_data(data, allowance, batch_size):
    '''
    data.pkl is a dict := {'x': [numpy array], 'y': [numpy array]} (on one client)
    returns x, y, which are both numpy array of length: batch_size
    '''
    data_x = data['x']
    data_y = data['y']
    if len(data_x)<allowance:
        epoch = math.ceil(allowance / len(data_x))
        data_x = np.repeat(data_x, epoch, axis=0)
        data_y = np.repeat(data_y, epoch, axis=0)
    ...
    data_x = data_x[:allowance]
    data_y = data_y[:allowance]
    #res = allowance % batch_size
    # this one is not sgd, but mini-batch
    # loop through mini-batches
    # for i in range(0, len(data_x), batch_size):
    #     if i == 0 and res != 0:
    #         batched_x = data_x[0:res]
    #         batched_y = data_y[0:res]
    #     else:
    #         batched_x = data_x[i + res:i + res + batch_size]
    #         batched_y = data_y[i + res:i + res + batch_size]
    #     yield (batched_x, batched_y)
    for i in reversed(range(0, len(data_x), batch_size)):
        batched_x = data_x[i:i + batch_size]
        batched_y = data_y[i:i + batch_size]
        # if i == 0 and res != 0:
        #     batched_x = data_x[0:res]
        ...
        #     batched_y = data_y[0:res]
        # else:
        #     batched_x = data_x[i - (batch_size-res):i + res]
        #     batched_y = data_y[i - (batch_size-res):i + res]
        yield (batched_x, batched_y)
...
def read_data(train_data_dir, test_data_dir):
    '''parses data.pkl in given train and test data.pkl directories

    assumes:
    - the data.pkl in the input directories are .json files with
    ...
    Return:
        ...
        train_data: dictionary of train data.pkl
        test_data: dictionary of test data.pkl
    '''
    clients = []
    groups = []
    train_data = {}
...
class Metrics(object):
    ...
    def write(self):
        ...
        metrics_dir = os.path.join('out', self.params['dataset'],
                                   'metrics_{}_{}_{}_{}_{}.json'.format(self.params['seed'], self.params['optimizer'],
                                                                        self.params['learning_rate'],
                                                                        self.params['num_epochs'], self.params['mu']))
