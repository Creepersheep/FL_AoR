...
class Model(object):
    ...
    def create_model(self, optimizer):
        ...
        #logits = tf.layers.dense(inputs=features, units=self.num_classes, kernel_regularizer=tf.contrib.layers.l2_regularizer(self.reg))
        # logits = tf.layers.dense(inputs=features, units=self.num_classes,
        #                          kernel_regularizer=tf.contrib.layers.l2_regularizer(self.regValue))
        logits = tf.layers.dense(inputs=features, units=self.num_classes,
    ...
    def set_params(self, model_params=None):
        if model_params is not None:
            with self.graph.as_default():
                ...
                for k, value in zip(all_vars, model_params):
                    k.load(value, self.sess)
    ...
    def solve_sgd(self, data, num_epochs=1, batch_size=32):
        '''Solves local optimization problem'''
        for _ in range(num_epochs):
            for X, y in batch_data(data, batch_size):
                with self.graph.as_default():
    ...
    # def solve_iters(self, data.pkl, num_iters=1, batch_size=32):
    #     '''Solves local optimization problem'''
    #
    #     for X, y in batch_data_multiple_iters(data.pkl, batch_size, num_iters):
    #         with self.graph.as_default():
    #             self.sess.run(self.train_op, feed_dict={self.features: X, self.labels: y})
    #     soln = self.get_params()
    #     comp = 0
