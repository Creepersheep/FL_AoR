...
class Model(object):
    ...
    def create_model(self, optimizer):
        ...
        # logits = tf.layers.dense(inputs=features, units=self.num_classes, kernel_regularizer=tf.contrib.layers.l2_regularizer(0.1))
        # logits = tf.layers.dense(inputs=features, units=self.num_classes,
        #                          kernel_regularizer=tf.keras.regularizers.l2(self.regValue))
        # regularizer must be less than 1
        # if self.regValue==1:
        #     logits = tf.layers.dense(inputs=features, units=self.num_classes,
