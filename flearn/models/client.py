...
class Client(object):
    ...
    def solve_bgd(self, num_allowance, batch_size):
        '''Solves local optimization problem with sgd
        ...
        '''
        bytes_w = self.model.size
        soln, comp = self.model.solve_bgd(self.train_data, num_allowance, batch_size)
        bytes_r = self.model.size
        return [self.num_samples, soln], (bytes_w, comp, bytes_r)

    def solve_sgd(self, num_epochs=1, batch_size=10):
        '''Solves local optimization problem with sgd
        ...
        '''
        bytes_w = self.model.size
        soln, comp = self.model.solve_sgd(self.train_data, num_epochs, batch_size)
        bytes_r = self.model.size
        return [self.num_samples, soln], (bytes_w, comp, bytes_r)
