'''
samples from all raw data.pkl;
by default samples in a non-iid manner; namely, randomly selects users from 
raw data.pkl until their cumulative amount of data.pkl exceeds the given number of
datapoints to sample (specified by --fraction argument);
ordering of original data.pkl points is not preserved in sampled data.pkl
'''
...
parser.add_argument('--fraction',
                help='fraction of all data.pkl to sample; default: 0.1;',
                type=float,
                default=0.1)
parser.add_argument('--u',
                help=('number of users in iid data.pkl set; ignored in niid case;'
...
print('------------------------------')
print('sampling data.pkl')

parent_path = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
data_dir = os.path.join(parent_path, 'data.pkl', args.name, 'data.pkl')
